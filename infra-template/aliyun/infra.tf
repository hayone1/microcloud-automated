...
resource "ssh_resource" "hosts-data" {
  ...
  commands    = [
    <<EOF
      ...
      ceph_volume_paths=$(lsscsi --scsi | awk '{print $1, $7}' | grep :${try(azurerm_virtual_machine_data_disk_attachment.ceph-disks-attachment[count.index].lun, "_none")}] | awk '{print $2}') &&
