...
resource "azurerm_network_interface" "interfaces" {
  count                 = local.provider_config.quantity
  name                  = "microcloud-interface"
  location              = azurerm_resource_group.rg.location
  resource_group_name   = azurerm_resource_group.rg.name

  ip_configuration {
    name                = "microcloud-interface-ipconfig"
...
resource "azurerm_managed_disk" "local-volume" {
...
resource "azurerm_managed_disk" "ceph-volume" {
...
resource "azurerm_virtual_machine_data_disk_attachment" "local-disks-attachment" {
  ...
  lun                   = (17 + count.index) #should be unique number
  caching               = var.storage-caching 
}
...
resource "ssh_resource" "hosts-data" {
  ...
  commands    = [
    <<EOF
      local_volume_path=$(df | awk '{print $1, $6}' | grep ${try(azurerm_managed_disk.local-volumes[count.index].name, "_none")} | awk '{print $1}') &&
      ceph_volume_path=$(df | awk '{print $1, $6}' | grep ${try(azurerm_managed_disk.ceph-volumes[count.index].name, "_none")} | awk '{print $1}') &&
